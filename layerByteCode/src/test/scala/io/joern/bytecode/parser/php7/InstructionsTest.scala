...
import io.joern.bytecode.parser.php7.instructions.ControlConstructs.{parseNumberDestinationPattern, parseStringDestinationPattern}
...
class InstructionsTest extends AnyWordSpec with Matchers {
  ...
  "parser getOperation" should {
    ...
    "be able to parse JMP L1" in {
      val operation = "JMP L1"
      val Parsed.Success(result, length) = parse(operation, getOperation(_))
      length shouldBe operation.length
      result.op match {
        case SingleValueOperation(code, value) =>
    ...
    "be able to parse JMPZ CV($var) L1" in {
      val operation = "JMPZ CV($var) L1"
      val Parsed.Success(result, length) = parse(operation, getOperation(_))
      length shouldBe operation.length
      result.op match {
        case DualValueOperation(code, lhs, rhs) =>
    ...
    "be able to parse JMPNZ CV($var) L1" in {
      val operation = "JMPNZ CV($var) L1"
      val Parsed.Success(result, length) = parse(operation, getOperation(_))
      length shouldBe operation.length
      result.op match {
        case DualValueOperation(code, lhs, rhs) =>
    ...
    "be able to parse JMPNZ_EX CV($var) L1" in {
      val operation = "JMPNZ_EX CV($var) L1"
      val Parsed.Success(result, length) = parse(operation, getOperation(_))
      length shouldBe operation.length
      result.op match {
        case DualValueOperation(code, lhs, rhs) =>
    ...
    "be able to parse JMP_NULL CV0($a) L4" in {
      val operation = "JMP_NULL CV0($a) L4"
      val Parsed.Success(result, length) = parse(operation, getOperation(_))
      length shouldBe operation.length
      result.op match {
        case DualValueOperation(code, lhs, rhs) =>
    ...
    "be able to parse FE_RESET_R V1 L1" in {
      val operation = "FE_RESET_R V1 L1"
      val Parsed.Success(result, length) = parse(operation, getOperation(_))
      length shouldBe operation.length
      result.op match {
        case DualValueOperation(code, lhs, rhs) =>
    ...
    "be able to parse FE_FETCH_R V1 CV($array) L1" in {
      val operation = "FE_FETCH_R V1 CV($var) L1"
      val Parsed.Success(result, length) = parse(operation, getOperation(_))
      length shouldBe operation.length
      result.op match {
    ...
    "be able to parse INSTANCEOF CV0($instance) (static) (no-autolod) (exception)" in {
      val op = "INSTANCEOF CV0($instance) (static) (no-autolod) (exception)"
  ...
  "parser getInstruction" should {
    ...
    "be able to parse SWITCH_STRING CV2($sType) \"to\": L13" in {
      val operation = s"""SWITCH_STRING CV2($$sType) "${encode("to")}": L13"""
      val Parsed.Success(result, length) = parse(operation, getInstruction(_))
      length shouldBe operation.length
      result match {
        case op: Operation =>
    ...
    "be able to parse SWITCH_STRING CV2($sType) \"to\": L13, \"bcc\": L14" in {
      val operation = s"""SWITCH_STRING CV2($$sType) "${encode("to")}": L13, "${encode("bcc")}": L14"""
      val Parsed.Success(result, length) = parse(operation, getInstruction(_))
      length shouldBe operation.length
      result match {
        case op: Operation =>
    ...
    "be able to parse SWITCH_STRING string(\"cl\") \"cgi\": L51, \"cgi-fcgi\": L51, default: L69" in {
      val operation = s"""SWITCH_STRING string("${encode("cl")}") "${encode("cgi")}": L51, "${encode("cgi-fcgi")}": L51, default: L69"""
      val Parsed.Success(result, length) = parse(operation, getInstruction(_))
      length shouldBe operation.length
      result match {
        case op: Operation =>
    ...
    "be able to parse SWITCH_STRING CV2($sType) \"to\": 0013, \"bcc\": L14, default: L16" in {
      val operation =
        s"""SWITCH_STRING CV2($$sType) "${encode("to")}": L13, "${encode("bcc")}": L14, default: L16"""
      val Parsed.Success(result, length) = parse(operation, getInstruction(_))
      length shouldBe operation.length
      result match {
        case op: Operation =>
    ...
    "be able to parse SWITCH_LONG CV($test) 33: L13, 44: L14, default: L23" in {
      val operation = "SWITCH_LONG CV($test) 33: L13, 44: L14, default: L23"
      val Parsed.Success(result, length) = parse(operation, getInstruction(_))
      length shouldBe operation.length
      result match {
        case op: Operation =>
    ...
    "be able to parse CATCH string(\"Exception\") L66" in {
      val operation = s"""CATCH string("${encode("Exception")}") L66"""
      val Parsed.Success(result, length) = parse(operation, getInstruction(_))
      length shouldBe operation.length
      result match {
        case op: Operation =>
    ...
    "be able to parse FE_FETCH_RW V6 CV2($frameLine) L29" in {
      val operation = "FE_FETCH_RW V6 CV2($frameLine) L29"
      val Parsed.Success(result, length) = parse(operation, getInstruction(_))
      length shouldBe operation.length
      result match {
        case op: Operation =>
    ...
    "be able to parse FE_RESET_R array(...) L267" in {
      val operation = "FE_RESET_R array(...) L267"
      val Parsed.Success(result, length) = parse(operation, getInstruction(_))
      length shouldBe operation.length
      result match {
        case op: Operation =>
    ...
    "be able to parse DECLARE_LAMBDA_FUNCTION 0" in{
      val op = "DECLARE_LAMBDA_FUNCTION int(0)"
      val Parsed.Success(result, length) = parse(op, getInstruction(_))
      length shouldBe op.length
      result shouldBe Operation(SingleValueOperation("DECLARE_LAMBDA_FUNCTION", IntegerLiteral(0)))
    }
    "be able to parse TYPE_CHECK bool(false) V2" in {
      val op = "TYPE_CHECK bool(false) V2"
      val Parsed.Success(result, length) = parse(op, getInstruction(_))
      length shouldBe op.length
  ...
  "parseDestinationPattern" should {
    "parse \"to\": L13" in {
      val string = s""""${encode("to")}": L13"""
      val Parsed.Success(result, length) =
        parse(string, parseStringDestinationPattern(_))
      length shouldBe string.length
      result shouldBe (("to", "13"))
    }
    "parse default: L13" in {
      val string = "default: L13"
      val Parsed.Success(result, length) =
        parse(string, parseStringDestinationPattern(_))
      length shouldBe string.length
      result shouldBe (("default", "13"))
    }
    "parse 44: L13" in {
      val string = "44: L13"
      val Parsed.Success(result, length) =
      ...
      length shouldBe string.length
      result shouldBe (("44", "13"))
    }
    "parse default: L16" in {
      val string = "default: L16"
      val Parsed.Success(result, length) =
        parse(string, parseNumberDestinationPattern(_))
      length shouldBe string.length
      result shouldBe (("default", "16"))
    }
  }
}
