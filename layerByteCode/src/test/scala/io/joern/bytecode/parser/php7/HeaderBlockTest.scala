...
class HeaderBlockTest extends AnyWordSpec with Matchers {
   ...
   "parsing '$_main:\n  ; (lines=42, args=43, vars=44, tmps=45)'" should {
    ...
    "be completely parsable" in {
      val Parsed.Success(result, _) =
        parse("$_main: ; (lines=42, args=43, vars=44, tmps=45)",
  ...
  "parser getMetaLineParsing " should {
    "be able to parse '; (before optimizer)'" in {
      ...
      assert(result.metaInfo.head == "before")
  ...
  "parser parseHeaderBlock" should {
    "be able to parse a valid header block with colon in path" in {
      val headerBlock =
        """$_main: ; (lines=23, args=0, vars=2, tmps=8)
          ; (before optimizer)
...
""".stripMargin
    ...
    "be able to parse a valid header block with class definition" in {
      val headerBlock =
        """Basic::__init__: ; (lines=23, args=0, vars=2, tmps=8)
  ; (before optimizer)
  ; /home/simon/tmp/bytecode-cpg/trivial-try-catch.php:1-23
"""
      val Parsed.Success(result, length) = parse(headerBlock, getHeaderBlock(_))
    ...
    "be able to parse a valid header block with namespace and class" in {
      val headerBlock =
        """namespace\Basic::__init__: ; (lines=23, args=0, vars=2, tmps=8)
  ; (before optimizer)
  ; /home/simon/tmp/bytecode-cpg/trivial-try-catch.php:1-23
"""
      val Parsed.Success(result, length) = parse(headerBlock, getHeaderBlock(_))
    ...
    "be able to parse a valid header block with namespace" in {
      val headerBlock =
        """namespace\__init__: ; (lines=23, args=0, vars=2, tmps=8)
  ; (before optimizer)
  ; /home/simon/tmp/bytecode-cpg/trivial-try-catch.php:1-23
"""
      val Parsed.Success(result, length) = parse(headerBlock, getHeaderBlock(_))
    ...
    "be able to parse a valid header block with deep namespace" in {
      val headerBlock =
        """start\namespace\__init__: ; (lines=23, args=0, vars=2, tmps=8)
  ; (before optimizer)
  ; /home/simon/tmp/bytecode-cpg/trivial-try-catch.php:1-23
"""
      val Parsed.Success(result, length) = parse(headerBlock, getHeaderBlock(_))
    ...
    "be able to parse valid header block from other unit test" in {
      val headerBlock =
        """$_main: ; (lines=3, args=0, vars=0, tmps=1)
          |    ; (before optimizer)
          |    ; /tmp/php2cpg17511137408163283203/test.php:1-9
          |""".stripMargin
      val Parsed.Success(_,length) = parse(headerBlock, getHeaderBlock(_))
      length shouldBe headerBlock.length
    }
  }
