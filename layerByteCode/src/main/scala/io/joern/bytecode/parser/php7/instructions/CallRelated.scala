...
import io.joern.bytecode.parser.php7.Literals.{
  getAnyLiteral,
  getByteCodeKeyword,
  getStringLiteral
}
...
object CallRelated {
  ...
  def parseCheckFuncArg[_: P]: P[String] =
    P("CHECK_FUNC_ARG " ~/ anyNumber.rep.!)
  def getCheckFuncArg[_: P]: P[SingleValueOperation] =
    P(parseCheckFuncArg.map(x =>
      SingleValueOperation("CHECK_FUNC_ARG", IntegerLiteral(x.toLong))))
